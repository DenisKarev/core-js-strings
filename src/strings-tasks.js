...
function sumOfCodes(str) {
  if (str) return str.split('').reduce((s, v) => s + v.charCodeAt(0), 0);
  return 0;
}
...
function countVowels(str) {
  ...
  if (!str) return 0;
  return str.split('').reduce((a, v) => (vowels.includes(v) ? a + 1 : a), 0);
}
...
function isPalindrome(str) {
  let reverse = str.split('').reverse().join('').toLocaleLowerCase();
  reverse = reverse.replaceAll(/[!,.?'" ]/g, '');
  return reverse === str.toLocaleLowerCase().replaceAll(/[!,.?'" ]/g, '');
}
...
function findLongestWord(sentence) {
  return sentence.split(' ').reduce((a, v) => (v.length > a.length ? v : a));
}
